...
public class ProcessorFactory {
	...
	public static IServerProcessor createProcessor(String resource,
		...
		if (resource.startsWith(debugUrl)) {
			return new DebugProcessor(resource, postData, response,
					requestHeader);
		} else {
			URL url = computeRemoteURL(resource);
	 ...
	 * @return
	 */
	private static URL computeRemoteURL(String resource) {
		String path = resource.replace(JsDebugServer.getLocalBaseUrl(), "");
		try {
